...
import { transferStockAction } from "@/app/inventario/actions";
...
export function StockTransferForm({
  ...
  return (
    ...
    >
      ...
      <Field label="Desde" error={state.fieldErrors?.fromBranchId?.[0]}>
        <select name="fromBranchId" required className="input" defaultValue="">
          <option value="" disabled>
            Seleccionar…
          </option>
          {branches.map((b) => (
      ...
      <Field label="Hacia" error={state.fieldErrors?.toBranchId?.[0]}>
        <select name="toBranchId" required className="input" defaultValue="">
          <option value="" disabled>
            Seleccionar…
          </option>
          {branches.map((b) => (
      ...
      <Field label="Nota (opcional)">
        <input
          name="note"
          className="input"
          placeholder="Ej. reposición sucursal"
        />
      </Field>
      ...
      >
        {pending ? "Trasladando…" : "Registrar traslado"}
      </button>
    </form>
  );
}
